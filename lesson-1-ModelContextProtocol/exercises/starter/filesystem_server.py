...
Usage::
    ...
    python filesystem_server.py [base_directory]

This will start an MCP server on standard input/output.  The server
descriptor will be named "filesystem" when discovered by MCP clients.

The implementation follows the pattern suggested in the LangGraph MCP
documentation: create a `FastMCP` instance, decorate functions with
`@mcp.tool()` to expose them, and finally call `mcp.run()` to start
serving【933798328899630†L214-L229】.
"""
